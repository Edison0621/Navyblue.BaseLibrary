...
| `Navyblue.Foundation.Testing` | `net6.0;net7.0;net8.0;net9.0;net10.0` | 测试时钟、测试用户、测试租户、内存事件收集等测试辅助 |
...
在集成测试里用 `FakeCurrentUser` / `FakeCurrentTenant` 替换 `HttpCurrentUser` / `HttpCurrentTenant`；用 `TestClock` 固定时间；用 `TestClaimsPrincipal` 构造带角色与租户的 Claims。
